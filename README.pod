...
Quick and dirty installation instructions for Debian.
  ...
  # Server
  apt-get install -y libnet-frame-perl libmath-numbercruncher-perl libnet-async-ping-perl \
    libnet-async-http-perl libio-async-perl libio-socket-timeout-perl libdbix-class-migration-perl \
    libpod-parser-perl libdbix-class-helpers-perl libdbd-mysql-perl libnet-telnet-perl \
    libipc-run-perl libio-async-ssl-perl libdancer2-perl libdancer2-plugin-dbic-perl \
    libmoox-singleton-perl libnet-async-smtp-perl liblinux-inotify2-perl libanyevent-perl \
    mariadb-server
  ...
  # Install checker on server
  cp etc/admonitor-checker.service /etc/systemd/system/
...
example, to collect the data every 15 minutes:
  ...
  5,20,35,50 * * * * DANCER_ENVIRONMENT=production /srv/admonitor/bin/get-agent-data.pl
