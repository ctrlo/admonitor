...
{   my ($self, $data) = @_;
    ...
    return $self->send_alarm("Backup verification not within 36 hours")
        if DateTime->now->delta_ms($parsed->{time})->in_units('hours') > 36;
}

sub parse_backup_status
{   my ($self, $status) = @_;

    if ($status =~ m!^File (.*) retrieved at (.*) with differences(.*)and /root/testfile (differ|are identical)\s?$!)
    {
        my $params = {};
